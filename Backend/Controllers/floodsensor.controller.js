    ...
    try {
      ...
      const deviceDataPromises = devices.map(async (device) => {
        try {
          ...
          return { deviceId: device._id, name: device.name, data: [], error: "Failed to fetch data" };
        }
      });
...
const getFieldData = async (req, res) => {
  try {
    ...
    const response = await axios.get(`${BASE_URL}/${device.thingSpeakChannelId}/fields/${field}.json`, {
    ...
    res.status(500).json({ success: false, message: "Failed to retrieve field data" });
  }
};
...
const getChannelStatus = async (req, res) => {
  try {
    ...
    const response = await axios.get(`${BASE_URL}/${device.thingSpeakChannelId}/status.json`, {
      params: { api_key: API_KEY },
    });
    ...
    res.status(500).json({ success: false, message: "Failed to retrieve channel status" });
  }
};
