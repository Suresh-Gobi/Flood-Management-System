...
export default function ReqOTP() {
  ...
  const onFinish = async (values) => {
    try {
      ...
      openNotification("success", "OTP Sent", response.data.message || "OTP has been sent to your email.");
    } catch (error) {
      console.error("Error:");
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <Card title={<h1 className="text-2xl">Request OTP</h1>} className="w-full max-w-md p-5 rounded-lg shadow-md">
        <Form name="otpRequest" layout="vertical" onFinish={onFinish} autoComplete="off">
          <Form.Item
            label="Email"
            name="email"
            rules={[
